...
public class TxBatchingSerialGatewaySenderQueueTest {
  ...
  public void setup() {
    ...
    InternalRegionFactory<?, ?> regionFactory =
        mock(InternalRegionFactory.class, RETURNS_DEEP_STUBS);
    when(regionFactory.setInternalMetaRegion(any())
        ...
        .setSerialGatewaySender(any())).thenReturn(uncheckedCast(regionFactory));
    when(regionFactory.create(QUEUE_REGION)).thenReturn(uncheckedCast(region));
  ...
  @Test
  public void peekGetsExtraEventsWhenNotAllEventsForTransactionsInMaxSizeBatch() {
  ...
  @Test
  public void peekGetsExtraEventsWhenNotAllEventsForTransactionsInBatchByTime() {
  ...
  private GatewaySenderEventImpl createMockGatewaySenderEventImpl(int transactionId,
    ...
    when(event.getRegion()).thenReturn(uncheckedCast(region));
    when(event.getMetadata())
        .thenReturn(new TxBatchMetadata(new TXId(null, transactionId), isLastEventInTransaction));
    return event;
  }
  ...
  private LocalRegion createLocalRegionMock() {
    ...
    Map<Long, GatewaySenderEventImpl> map = new HashMap<>();
    map.put(0L, event1);
    map.put(1L, event2);
    map.put(2L, event3);
    map.put(3L, event4);
