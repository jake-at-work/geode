...
public class ResourceManagerValidator {
  ...
  private static void validateSunArguments(GemFireCache cache, ResourceManager rm,
    ...
    if (cache.getLogger().fineEnabled()) {
      final var builder = "Relevant input java arguments: " + "dashXms=" + dashXms
  ...
  private static void validateJavaHeapParameters(GemFireCache cache, String dashXms,
    ...
    if (dashXms == null) {
      ...
      var dashXmsList = splitAtDigits(dashXms);
      var dashXmsStr = dashXmsList.get(1);
      var dashXmxList = splitAtDigits(dashXmx);
      var dashXmxStr = dashXmxList.get(1);
      if (!dashXmsStr.equals(dashXmxStr)) {
        final var builder = "Setting the initial (" + dashXmsStr + dashXmsList.get(2)
  ...
  private static void verifyCMSInitiatingOccupancyFraction(GemFireCache cache, ResourceManager rm,
    ...
    if (cmsIOF == null) {
      ...
      // thresholds.
      var cmsIOFVal = Integer.parseInt(cmsIOF.split("=")[1]);
  ...
  private static List<String> splitAtDigits(String input) {
    var matcher = DIGIT_PATTERN.matcher(input);
