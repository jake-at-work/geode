...
import org.apache.geode.redis.internal.commands.executor.sortedset.AbstractSortedSetRangeOptions;
...
import org.apache.geode.redis.internal.commands.executor.sortedset.ZAggregator;
  ...
  public Object zadd(Region<RedisKey, RedisData> region, RedisKey key, List<byte[]> membersToAdd,
    ...
    for (var i = 0; i < membersToAdd.size(); i++) {
      ...
      if (!addResult.equals(MemberAddResult.NO_OP)) {
        if (deltaInfo == null) {
          deltaInfo = new AddByteArrayDoublePairs(membersToAdd.size());
        }
