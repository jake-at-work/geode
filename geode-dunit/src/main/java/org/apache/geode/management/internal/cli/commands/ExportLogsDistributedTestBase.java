...
public class ExportLogsDistributedTestBase {
  ...
  public void setup() throws Exception {
    ...
    for (var member : expectedMessages.keySet()) {
      var logLines = expectedMessages.get(member);
  ...
  public void withFiles_savedToLocatorSpecifiedRelativeDir() throws Exception {
    var extensions = new String[] {"zip"};
    var workingDirPath = getWorkingDirectory().toPath();
    var subdirPath = workingDirPath.resolve("some").resolve("test").resolve("directory");
    var relativeDir = workingDirPath.relativize(subdirPath);
    // Expects locator to produce file in own working directory when connected via JMX
  ...
  public void startAndEndDateCanIncludeLogs() throws Exception {
    ...
    var acceptedLogLevels = Stream.of("info", "error", "debug").collect(toSet());
    verifyZipFileContents(acceptedLogLevels);
  }

  @Test
  public void testExportWithStartAndEndDateTimeFiltering() throws Exception {
    var cutoffTime = LocalDateTime.now().atZone(ZoneId.systemDefault());
    ...
    var messageAfterCutoffTime =
        "[this message should not show up since it is after cutoffTime]";
    var logLineAfterCutoffTime = new LogLine(messageAfterCutoffTime, "info", true);
    server1.invoke(() -> {
      var logger = LogService.getLogger();
    ...
    var cutoffTimeString = dateTimeFormatter.format(cutoffTime);
  ...
  public void testExportWithThresholdLogLevelFilter() throws Exception {
    ...
    var acceptedLogLevels = Stream.of("info", "error").collect(toSet());
    verifyZipFileContents(acceptedLogLevels);
  ...
  public void testExportWithExactLogLevelFilter() throws Exception {
    gfshConnector.executeAndAssertThat("export logs --log-level=info --only-log-level=true")
        .statusIsSuccess();
    var acceptedLogLevels = Stream.of("info").collect(toSet());
    verifyZipFileContents(acceptedLogLevels);
  }
  ...
  private void verifyZipFileContents(Set<String> acceptedLogLevels) throws Exception {
    var unzippedLogFileDir = unzipExportedLogs();
  ...
  private void verifyLogFileContents(Set<String> acceptedLogLevels, File dirForMember)
    ...
    var memberName = dirForMember.getName();
    var member = expectedMessages.keySet().stream()
    ...
    var logFileForMember = new File(dirForMember, memberName + ".log");
    assertThat(logFileForMember).exists();

    var logFileContents = FileUtils.readLines(logFileForMember, Charset.defaultCharset())
        .stream().collect(joining("\n"));

    for (var logLine : expectedMessages.get(member)) {
      var shouldExpectLogLine =
  ...
  private File unzipExportedLogs() throws Exception {
    var locatorWorkingDir = getWorkingDirectory();
    var filesInDir = Stream.of(getWorkingDirectory().listFiles()).collect(toList());
    assertThat(filesInDir).isNotEmpty();

    var zipFilesInDir = Stream.of(getWorkingDirectory().listFiles())
    ...
    var unzippedLogFileDir = temporaryFolder.newFolder("unzippedLogs");
