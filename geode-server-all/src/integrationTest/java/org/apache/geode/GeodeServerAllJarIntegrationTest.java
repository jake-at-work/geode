...
public class GeodeServerAllJarIntegrationTest {
  ...
  public void verifyManifestClassPath() throws IOException {
    ...
    assertThat(getManifestClassPath())
        .describedAs("The geode-server-all.jar's manifest classpath has changed. Verify "
            + "dependencies and copy geode-server-all/build/integrationTest/dependency_classpath.txt "
            + "to geode-server-all/src/integrationTest/resources/dependency_classpath.txt")
