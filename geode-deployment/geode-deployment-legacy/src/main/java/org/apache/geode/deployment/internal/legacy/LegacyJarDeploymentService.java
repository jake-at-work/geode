...
public class LegacyJarDeploymentService implements JarDeploymentService {
  ...
  public synchronized ServiceResult<Deployment> deploy(Deployment deployment) {
    try {
      var deployedJar = jarDeployer.deploy(deployment.getFile());
      ...
      if (deployedJar == null) {
        ...
        var deploymentCopy = new Deployment(deployment, deployedJar.getFile());
  ...
  public synchronized ServiceResult<Deployment> undeployByFileName(String fileName) {
    try {
      ...
      var artifactId = JarFileUtils.getArtifactId(fileName);

      var deployment = deployments.get(artifactId);
      if (deployment == null || !deployment.getFileName().equals(fileName)) {
        return Failure.of(fileName + " not deployed");
      }

      var removedDeployment = deployments.remove(artifactId);
  ...
  public void loadJarsFromWorkingDirectory() {
    ...
    var latestVersionOfJarsOnDisk = jarDeployer.getLatestVersionOfJarsOnDisk();
    try {
      List<Deployment> deploymentList = new LinkedList<>();
      for (var entry : latestVersionOfJarsOnDisk.entrySet()) {
        var deployedJar = entry.getValue();
        ...
        var deployment = createDeployment(deployedJar);
  ...
  @Override
  public void close() {
    var jarNames = deployments.keySet().toArray(new String[] {});
    logger.debug(
  ...
  private Deployment createDeployment(DeployedJar deployedJar) {
    var deployment = new Deployment(deployedJar.getArtifactId(), "",
  ...
  private Deployment createDeployment(File deployedJar) {
    var deployment = new Deployment(deployedJar.getName(), "",
  ...
  private synchronized void flushCaches() {
    try {
      var typeRegistry = ((InternalCache) CacheFactory.getAnyInstance()).getPdxRegistry();
