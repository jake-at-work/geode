...
public class LikePredicateJUnitTest {
  ...
  private void equalityForm_2(boolean useBindPrms) throws Exception {
    ...
    var rs1 = new SelectResults[][] {{results, expectedResults1}};
  ...
  private void regexMetaChar(boolean useBindPrms) throws Exception {
    ...
    var values = new String[] {"active", "act**ve", "ac+t+ve", "?act?ve", "act)ve^",
  ...
  private void enhancedLike(boolean useBindPrms) throws Exception {
    ...
    var likePredicates = new String[] {"active", "act%%ve", "a%e", "%ctiv%", "%c%iv%",
        "%ctive", "%%ti%", "activ_", "_ctive", "ac_ive", "_c_iv_", "_ctiv%", "__tive", "act__e",
        "a%iv_", "a_tiv%", "%", "ac%",};
    for (final var predicate : likePredicates) {
      if (!useBindPrms) {
        q = qs.newQuery(
    ...
    for (final var likePredicate : likePredicates) {
      if (!useBindPrms) {
        q = qs.newQuery(
  ...
  private void enhancedLike2(boolean useBindPrms) throws Exception {
    ...
    var values = new String[] {"ac\\tive", "X\\\\X", "Y%Y", "Z\\%Z", "pass\\ive",
    ...
    var result = new String[] {"ac\\tive", "ac\\tive", "ac\\tive", "Y%Y", "X\\\\X", "X\\\\X",
        "Z\\%Z", "inact\\%+ive", "1inact\\_+ive",};
    var likePredicates = new String[] {"ac\\\\tive", "ac\\\\%", "ac_tive", "Y\\%Y", "X__X",
        "X%X", "Z\\\\\\%Z", "inact\\\\%+ive", "1inact\\\\_+ive",};
    for (var i = 0; i < likePredicates.length; i++) {
      if (!useBindPrms) {
        q = qs.newQuery(
  ...
  public void testLikeWithOtherIndexedField() throws Exception {
    ...
    var q = qs.newQuery(" SELECT  *  FROM " + SEPARATOR + "pos ps WHERE ps.status like '%b%'");
    var results = (SelectResults) q.execute();
    var expectedResultSize = 2;
    ...
    var expectedResults = (SelectResults) q.execute();
  ...
  private void enhancedNotLike(boolean useBindPrms, boolean useIndex) throws Exception {
    ...
    var values = new String[] {"active", "inactive",};
    ...
    var likePredicates1 = new String[] {"active", "act%%ve", "a%e", "activ_", "ac_ive",
        "act__e", "a%iv_", "a_tiv%", "ac%",};
    for (final var item : likePredicates1) {
      if (!useBindPrms) {
        q = qs.newQuery(
    ...
    var likePredicates2 = new String[] {"%ctiv%", "%c%iv%", "%ctive", "%%ti%", "%",};
    for (final var value : likePredicates2) {
      if (!useBindPrms) {
        q = qs.newQuery(
    ...
    var likePredicates3 = new String[] {"___ctive", "___c_iv_", "___ctiv%", "____tive",};
    for (final var s : likePredicates3) {
      if (!useBindPrms) {
        q = qs.newQuery(
  ...
  private void likeWithNewLine(boolean useIndex) throws Exception {
    ...
    var query = qs.newQuery("select * from " + SEPARATOR + "pos a where a like '%bc%'");
  ...
  private void removeIndexDuringQuery(boolean isPr) throws Exception {
    ...
    var query = "select distinct * from " + name + " where status like 'act%'";
  ...
  public void testQueryExecutionMultipleTimes() throws Exception {
    ...
    var region = cache.createRegion("pos", regionAttributes);
    var region2 = cache.createRegion("pos2", regionAttributes);

    // Create Index
    var i1 =
        cache.getQueryService().createIndex("status", IndexType.FUNCTIONAL, "ps.status",
            SEPARATOR + "pos ps");
    var i2 = cache.getQueryService().createIndex("description", IndexType.FUNCTIONAL,
        "ps.description", SEPARATOR + "pos ps");
    var i3 = cache.getQueryService().createIndex("description2", IndexType.FUNCTIONAL,
  ...
  private void executeInvalidQueryWithLike(boolean useBindParam) throws Exception {
    ...
    q[3] =
        qs.newQuery("SELECT distinct *  FROM " + SEPARATOR + "pos ps WHERE NOT (ps.status = "
            + predicate2 + ")");
    var results = new SelectResults[2][2];
    if (useBindParam) {
  ...
  public void testUndefined() throws Exception {
    ...
    var queries =
        new String[] {"select * from " + SEPARATOR + "pos where status.toUpperCase() like '%ACT%'",
    ...
    var res = new SelectResults[queries.length][2];
  ...
  private static class PortfolioModifiedStatus implements Serializable {
    public PortfolioModifiedStatus(int id) {
      var status = new String[] {"inactive", "active"};
    }
  }
}
