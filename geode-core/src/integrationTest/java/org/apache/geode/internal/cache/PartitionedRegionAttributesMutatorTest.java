...
public class PartitionedRegionAttributesMutatorTest {
  ...
  public void testChangeCacheLoaderDuringBucketCreation()
    ...
    startCluster();
    server.invoke(() -> {
      var mutationMade = new CountDownLatch(1);
      var bucketCreated = new CountDownLatch(1);
      var pr = createRegion(bucketCreated, mutationMade);
      var loader = createTestCacheLoader();

      var createBucket =
          CompletableFuture.runAsync(() -> PartitionRegionHelper.assignBucketsToPartitions(pr));
      bucketCreated.await();
      var beforeConfig = getConfig(pr);
  ...
  private static void verifyMetaDataIsOk(PartitionRegionConfig beforeConfig,
    ...
    var beforeMembers = getMembers(beforeConfig);
    var afterMembers = getMembers(afterConfig);
  ...
  public void testChangeCacheWriterWithClusterConfigUpdatesNodeInfo() {
    ...
    server.invoke(() -> {
      var pr = createRegionSpy();
      var writer = createTestCacheWriter();
      var beforeConfig = getConfig(pr);
  ...
  public void testChangeCustomEntryTtlDuringBucketCreation()
    ...
    server.invoke(() -> {
      ...
      var bucketCreated = new CountDownLatch(1);
      var pr = createRegion(bucketCreated, mutationMade);
      var customExpiry = createTestCustomExpiry();
  ...
  public void testChangeEntryIdleTimeoutDuringBucketCreation()
    ...
    server.invoke(() -> {
      var mutationMade = new CountDownLatch(1);
      var bucketCreated = new CountDownLatch(1);
      var pr = createRegionWithFewBuckets(bucketCreated, mutationMade);
  ...
  private static PartitionedRegion createRegion(CountDownLatch bucketCreated,
    ...
    var pr = (PartitionedRegion) ClusterStartupRule.getCache()
