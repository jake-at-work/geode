...
public class StatSamplerIntegrationTest {
  ...
  public void testStatSampler() throws Exception {
    ...
    var archiveFileName = folder.getAbsolutePath() + File.separator + getName() + ".gfs";
    ...
    final var stopper = new CancelCriterion() {
      @Override
    ...
    final var factory = new LocalStatisticsFactory(stopper);
    final var statsST1 =
    ...
    final var ST1 = factory.createType("ST1", "ST1", statsST1);
    final var st1_1 = factory.createAtomicStatistics(ST1, "st1_1", 1);
    ...
    var samplerStatsInstances = factory.findStatisticsByTextId("statSampler");
    assertNotNull(samplerStatsInstances);
    assertEquals(1, samplerStatsInstances.length);
    final var samplerStats = samplerStatsInstances[0];
    ...
    final var archiveFile = new File(System.getProperty("stats.archive-file"));
    assertTrue(archiveFile.exists());
    final var reader = new StatArchiveReader(new File[] {archiveFile}, null, false);

    var resources = reader.getResourceInstList();
    for (var resource : resources) {
      var ri = (StatArchiveReader.ResourceInst) resource;
  ...
  private void waitForStatSamplerToRun(final Statistics samplerStats, final int timesToRun) {
    final var startSampleCount = samplerStats.getInt("sampleCount");
  ...
  private void incDouble(Statistics statistics, String stat, double value) {
    ...
    var statValues = getOrCreateExpectedValueMap(statistics);
