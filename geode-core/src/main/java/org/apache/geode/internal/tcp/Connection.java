...
public class Connection implements Runnable {
  ...
  boolean checkForIdleTimeout() {
    ...
    if (socketInUse || sharedResource && !preserveOrder) {
  ...
  private void close(String reason, boolean cleanupEndpoint, boolean removeEndpoint,
    ...
    synchronized (outLock) {
      ...
      try (final ByteBufferSharing outputSharing = ioFilter.wrap(buffer)) {
        ...
        while (wrappedBuffer.remaining() > 0) {
          int amtWritten = 0;
          final long start = stats.startSocketWrite();
          try {
            amtWritten = channel.write(wrappedBuffer);
          } finally {
            stats.endSocketWrite(start, amtWritten, 0);
          }
        }
      }
    }
