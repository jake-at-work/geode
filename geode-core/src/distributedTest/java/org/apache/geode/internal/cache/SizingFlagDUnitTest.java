...
public class SizingFlagDUnitTest extends JUnit4CacheTestCase {
  ...
  public void testLargeDelta() {
    ...
    var BIG_DELTA_SIZE = 32 * 1024 * 2;
    var sb = new StringBuilder(BIG_DELTA_SIZE);
    for (var i = 0; i < BIG_DELTA_SIZE; i++) {
      sb.append('7');
    }
    var delta1 = new TestDelta(true, sb.toString());


    assignPRBuckets(vm0);
    var vm0isPrimary = prHostsBucketForKey(vm0, 0);
    if (!vm0isPrimary) {
  ...
  void doPRDeltaTestLRU(boolean shouldSizeChange, boolean heapLRU, boolean putOnPrimary,
    ...
    var delta1 = new TestDelta(false, "12345");
    if (putOnPrimary) {
    ...
    var origEvictionSize0 = getSizeFromEvictionStats(primaryVm);
    var origEvictionSize1 = getSizeFromEvictionStats(secondaryVm);
    var origPRSize0 = getSizeFromPRStats(primaryVm);
    var origPRSize1 = getSizeFromPRStats(secondaryVm);
    ...
    var finalEvictionSize0 = getSizeFromEvictionStats(primaryVm);
    var finalEvictionSize1 = getSizeFromEvictionStats(secondaryVm);
    var finalPRSize0 = getSizeFromPRStats(primaryVm);
    var finalPRSize1 = getSizeFromPRStats(secondaryVm);
    if (shouldSizeChange) {
  ...
  private void doListenerTestRR(VM vm0, VM vm1) {
    ...
    var origEvictionSize0 = getSizeFromEvictionStats(vm0);
    var origEvictionSize1 = getSizeFromEvictionStats(vm1);
    ...
    var finalEvictionSize0 = getSizeFromEvictionStats(vm0);
    var finalEvictionSize1 = getSizeFromEvictionStats(vm1);
  ...
  private void doListenerTestPR(VM vm0, VM vm1) {
    ...
    var origPRSize1 = getSizeFromPRStats(vm1);
    ...
    var finalEvictionSize0 = getSizeFromEvictionStats(vm0);
    var finalEvictionSize1 = getSizeFromEvictionStats(vm1);
    var finalPRSize0 = getSizeFromPRStats(vm0);
    var finalPRSize1 = getSizeFromPRStats(vm1);
  ...
  private void doRRMemLRUTest() {
    ...
    get(vm1, "a", v);
    var vSize = CachedDeserializableFactory.calcSerializedMemSize(v);

    assertValueType(vm1, "a", ValueType.CD_DESERIALIZED);
    var evictionSizeAfterGet = getSizeFromEvictionStats(vm1);
    assertEquals(1, getObjectSizerInvocations(vm1));
  ...
  private void doPRMemLRUTest() {
    ...
    var origEvictionSize0 = getSizeFromEvictionStats(vm0);
    var origEvictionSize1 = getSizeFromEvictionStats(vm1);
    var origPRSize0 = getSizeFromPRStats(vm0);
    var origPRSize1 = getSizeFromPRStats(vm1);
    ...
    assertValueType(vm1, 0, ValueType.CD_SERIALIZED);
    var evictionSizeAfterGet = getSizeFromEvictionStats(vm0);
    var prSizeAfterGet = getSizeFromPRStats(vm0);
    assertEquals(1, getObjectSizerInvocations(vm0));
    assertEquals(0, getObjectSizerInvocations(vm1));
    assertEquals(200000 + CachedDeserializableFactory.overhead() - vSize,
    ...
    var evictionSizeAfterPutVm1 = getSizeFromEvictionStats(vm1);
    ...
    var evictionSizeAfterGetVm1 = getSizeFromEvictionStats(vm1);
    assertValueType(vm0, 113, ValueType.CD_SERIALIZED);
    ...
    return (Long) vm0.invoke(new SerializableCallable() {
      ...
      public Object call() {
        ...
        if (region instanceof PartitionedRegion) {
          ...
          for (var i = 0; i < pr.getPartitionAttributes().getTotalNumBuckets(); i++) {
  ...
  private boolean prHostsBucketForKey(VM vm, final Object key) {
    var result = (Boolean) vm.invoke(new SerializableCallable("prHostsBucketForKey") {
      @Override
      public Object call() {
        Cache cache = getCache();
        var myId = cache.getDistributedSystem().getDistributedMember();
        Region region = cache.getRegion("region");
        var hostMember = PartitionRegionHelper.getPrimaryMemberForKey(region, key);
        if (hostMember == null) {
          throw new IllegalStateException("bucket for key " + key + " is not hosted!");
        }
        var res = myId.equals(hostMember);
  ...
  private long getSizeFromEvictionStats(LocalRegion region) {
    var result = region.getEvictionCounter();
  ...
  private void assertValueType(VM vm, final Object key, final ValueType expectedType) {
    vm.invoke(new SerializableRunnable("Create rr") {
      ...
      public void run() {
        Cache cache = getCache();
        var region = (LocalRegion) cache.getRegion("region");
        var value = region.getValueInVM(key);
        switch (expectedType) {
          case RAW_VALUE:
  ...
  private void createPR(VM vm, final boolean enableLRU) {
    vm.invoke(new SerializableRunnable("Create pr") {
      ...
      public void run() {
        ...
        var region = cache.createRegion("region", attr.create());
      }
    });
