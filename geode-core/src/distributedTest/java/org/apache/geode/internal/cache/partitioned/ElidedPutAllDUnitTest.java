...
public class ElidedPutAllDUnitTest extends JUnit4CacheTestCase {
  ...
  public void testElidedPutAllOnPR() throws Exception {
    ...
    vm1.invoke(new SerializableRunnable("perform conflicting update") {
      ...
      public void run() {
        Cache cache = getCache();
        var region = (PartitionedRegion) cache
        ...
        try {
          ...
          map.put(key, "value-3");
          var dpao = region.newPutAllOperation(map, null);
          var event = EntryEventImpl.create(region, Operation.PUTALL_CREATE, null, null,
          ...
          var successfulPuts = new VersionedObjectList(1, true, true);
