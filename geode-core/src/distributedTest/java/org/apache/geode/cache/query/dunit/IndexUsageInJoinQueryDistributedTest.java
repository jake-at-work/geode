...
public class IndexUsageInJoinQueryDistributedTest implements Serializable {
  ...
  public void testSecondIndexUsedWhenExecutingJoinQuery() throws Exception {
    ...
    // Load regions
    var numProducts = 1000;
    var numInstruments = 100;
  ...
  private void loadRegions(int numProducts, int numInstruments) {
    ...
    for (var i = 0; i < numInstruments; i++) {
      var instrumentId = String.valueOf(i);
      Map<String, String> types = new HashMap<>();
      types.put("TYPE", "TYPE1");
      var instrument = new Instrument(instrumentId, "0", types);
  ...
  private void executeQuery(int numInstruments) throws Exception {
    ...
    var query = ClusterStartupRule.getClientCache().getQueryService().newQuery(queryString);
    var results = (Collection) query.execute(new Object[] {"TYPE1"});
