...
public class AlterRegionCommand extends SingleGfshCommand {
  ...
  public ResultModel alterRegion(
    ...
    if (ccService != null) {
      for (var group : groups) {
        var clusterConfig = ccService.getCacheConfig(group);
    ...
    var deltaConfig = new RegionConfig();
    deltaConfig.setName(regionPath);
    var regionAttributesType = new RegionAttributesType();
    ...
    var regionAlterResults =
        executeAndGetFunctionResult(new RegionAlterFunction(), deltaConfig, targetMembers);
    var result = ResultModel.createMemberStatusResult(regionAlterResults);
  ...
  public boolean updateConfigForGroup(String group, CacheConfig cacheConfig, Object configObject) {
    var deltaConfig = (RegionConfig) configObject;
    var existingConfig = find(cacheConfig.getRegions(),
        deltaConfig.getId());

    var deltaAttributes = deltaConfig.getRegionAttributes();
    var existingAttributes = existingConfig.getRegionAttributes();
    ...
    var evictionAttributes = deltaAttributes.getEvictionAttributes();
    if (evictionAttributes != null) {
      // we only set the max in the delta's lruEntryCount in the alter region command
      var newMax = evictionAttributes.getLruEntryCount().getMaximum();
      var existingEviction = existingAttributes.getEvictionAttributes();
