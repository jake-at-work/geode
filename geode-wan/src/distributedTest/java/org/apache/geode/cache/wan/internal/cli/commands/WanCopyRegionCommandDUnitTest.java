...
import static org.apache.geode.management.internal.cli.functions.WanCopyRegionFunction.WAN_COPY_REGION__MSG__ALREADY__RUNNING__COMMAND;
import static org.apache.geode.management.internal.cli.functions.WanCopyRegionFunction.WAN_COPY_REGION__MSG__CANCELED__BEFORE__HAVING__COPIED;
import static org.apache.geode.management.internal.cli.functions.WanCopyRegionFunction.WAN_COPY_REGION__MSG__EXECUTION__CANCELED;
import static org.apache.geode.management.internal.cli.functions.WanCopyRegionFunction.WAN_COPY_REGION__MSG__NO__RUNNING__COMMAND;
import static org.apache.geode.management.internal.cli.functions.WanCopyRegionFunction.WAN_COPY_REGION__MSG__REGION__NOT__FOUND;
import static org.apache.geode.management.internal.cli.functions.WanCopyRegionFunction.WAN_COPY_REGION__MSG__SENDER__NOT__FOUND;
import static org.apache.geode.management.internal.cli.functions.WanCopyRegionFunction.WAN_COPY_REGION__MSG__SENDER__SERIAL__AND__NOT__PRIMARY;
import static org.apache.geode.management.internal.cli.functions.WanCopyRegionFunctionDelegate.WAN_COPY_REGION__MSG__COPIED__ENTRIES;
...
public class WanCopyRegionCommandDUnitTest extends WANTestBase {
  ...
  public void testUnsuccessfulExecution_RegionNotFound() throws Exception {
    var serversInA = Arrays.asList(vm5, vm6, vm7);
    var serverInB = vm3;
    var serverInC = vm4;
    var client = vm8;
    var senderIdInA = "B";
  ...
  public void testUnsuccessfulExecution_SenderNotFound() throws Exception {
    ...
    var wanCopyRegionBatchSize = 20;
    var regionName = getRegionName(true);
  ...
  public void testUnsuccessfulExecution_ExceptionAtReceiver(
    ...
    var senderLocatorPort = create3WanSitesAndClient(isPartitionedRegion, vm0,
        vm1, vm2, serversInA, serverInB, serverInC, client,
        senderIdInA, senderIdInB, true);

    var regionName = getRegionName(isPartitionedRegion);
    var wanCopyRegionBatchSize = 20;
    ...
    if (isParallelGatewaySender) {
      var command =
          verifyStatusIsError(gfsh.executeAndAssertThat(commandString));
      var exceptionError =
          new Condition<String>(s -> s.startsWith("Error ("), "Error");
      command
          .hasTableSection(ResultModel.MEMBER_STATUS_SECTION)
          .hasColumn("Message")
          .asList()
      ...
      var exceptionError =
          new Condition<String>(s -> s.startsWith("Error ("), "Error");
      var senderNotPrimary = new Condition<String>(
  ...
  public void testSenderOrReceiverGoesDownDuringExecution(boolean useParallel,
    ...
    final var wanCopyRegionBatchSize = 10;
    final int entries;
    ...
    final var regionName = getRegionName(usePartitionedRegion);
    ...
    var server2InA = vm2;
    var server3InA = vm3;
    var serversInA = Arrays.asList(server1InA, server2InA, server3InA);
    final var senderIdInA = "B";
    ...
    var server2InB = vm6;
    var server3InB = vm7;
    var serversInB = Arrays.asList(server1InB, server2InB, server3InB);
    var client = vm8;

    var locatorAPort = create2WanSitesAndClient(locatorInA, serversInA, senderIdInA, locatorInB,
    ...
    var wanCopyCommandCallable = (Callable<CommandResultAssert>) () -> {
      var command = new CommandStringBuilder(WAN_COPY_REGION)
          .addOption(WAN_COPY_REGION__REGION, regionName)
          .addOption(WAN_COPY_REGION__SENDERID, senderIdInA)
          .addOption(WAN_COPY_REGION__BATCHSIZE, String.valueOf(wanCopyRegionBatchSize))
  ...
  public void testRegionDestroyedDuringExecution(boolean isParallelGatewaySender,
    ...
    final var wanCopyRegionBatchSize = 10;
    final var entries = 1000;

    final var regionName = getRegionName(isPartitionedRegion);
    ...
    var result = wanCopyCommandFuture.get();
    ...
    var regionDestroyedError = new Condition<String>(
        s -> (s.startsWith(
  ...
  public void testDetectOngoingExecution(boolean useParallel,
    ...
    // Check command status and output
    var exceptionError = new Condition<String>(
    ...
    // cancel command
    var commandString1 = new CommandStringBuilder(WAN_COPY_REGION)
        .addOption(WAN_COPY_REGION__REGION, regionName)
        .addOption(WAN_COPY_REGION__SENDERID, senderIdInA)
        .addOption(WAN_COPY_REGION__BATCHSIZE, String.valueOf(wanCopyRegionBatchSize))
  ...
  public void testSuccessfulExecution(boolean isPartitionedRegion,
    ...
    if (isPartitionedRegion && isParallelGatewaySender) {
      var msg1 = CliStrings.format(WAN_COPY_REGION__MSG__COPIED__ENTRIES, 33);
      var msg2 = CliStrings.format(WAN_COPY_REGION__MSG__COPIED__ENTRIES, 34);
      command
          .hasTableSection(ResultModel.MEMBER_STATUS_SECTION)
          .hasColumn("Message")
          .containsExactlyInAnyOrder(msg1, msg1, msg2);
    } else {
      var msg1 = CliStrings.format(WAN_COPY_REGION__MSG__COPIED__ENTRIES, 100);
      var msg2 = CliStrings
  ...
  public void testSuccessfulExecutionWhileRunningOpsOnRegion(
    ...
    var wanCopyRegionBatchSize = 20;
    var entries = 10000;
    var keySet = LongStream.range(0L, entries).boxed().collect(Collectors.toSet());
    var regionName = getRegionName(isPartitionedRegion);
    // Put entries
    client.invoke(() -> doPutsFrom(regionName, 0, entries));
    ...
    var maxRate = (entries / 5) / (isParallelGatewaySender ? serversInA.size() : 1);
    ...
    // Check command status and output
    var command = wanCopyCommandFuture.get();
  ...
  public void testUnsuccessfulCancelExecution(boolean isPartitionedRegion,
    ...
    gfsh.connectAndVerify(senderLocatorPort, GfshCommandRule.PortType.locator);
    var cancelCommand = new CommandStringBuilder(WAN_COPY_REGION)
        .addOption(WAN_COPY_REGION__REGION, regionName)
        .addOption(WAN_COPY_REGION__SENDERID, senderIdInA)
        .addOption(WAN_COPY_REGION__CANCEL)
        .getCommandString();
    ...
    var msg1 = CliStrings.format(WAN_COPY_REGION__MSG__NO__RUNNING__COMMAND,
        regionName, senderIdInA);
    cancelCommandResult
        .hasTableSection(ResultModel.MEMBER_STATUS_SECTION)
        .hasColumn("Message")
  ...
  public void testSuccessfulCancelExecution(boolean isPartitionedRegion,
    ...
    var gfshCancelCommand = new GfshCommandRule();
    ...
    var wanCopyCommandResult = wanCopyCommandFuture.get();
    if (isPartitionedRegion && isParallelGatewaySender) {
      verifyStatusIsError(wanCopyCommandResult);
      var msg = WAN_COPY_REGION__MSG__CANCELED__BEFORE__HAVING__COPIED;
    ...
    if (useParallel) {
      ...
      for (var server : serversInA) {
        boolean senderWasStopped = server.invoke(() -> {
          var sender = cache.getGatewaySender(senderIdInA);
  ...
  public void verifyResultOfStoppingReceiverWhenUsingSerialSender(
    ...
    verifyStatusIsOk(command);
    var haveEntriesCopied =
        new Condition<String>(s -> s.startsWith("Entries copied: "), "Entries copied");
    var senderNotPrimary = new Condition<String>(
  ...
  public void verifyResultOfStoppingParallelSender(CommandResultAssert command) {
    ...
    var startsWithError = new Condition<String>(
  ...
  private void waitForWanCopyRegionCommandToStart(boolean useParallel, boolean usePartitionedRegion,
    ...
    final var executionsExpected = useParallel && usePartitionedRegion ? servers.size() : 1;
    await().untilAsserted(
  ...
  private int getCopiedEntries(CommandResultAssert command) throws ParseException {
    var messages = command.hasTableSection(ResultModel.MEMBER_STATUS_SECTION).getActual()
        .getContent().get("Message");
    var numberPattern = Pattern.compile("(.*)(copied: )(.*)");
    var copiedEntries = 0;
    for (var message : messages) {
      var m = numberPattern.matcher(message);
      if (m.find()) {
        copiedEntries +=
