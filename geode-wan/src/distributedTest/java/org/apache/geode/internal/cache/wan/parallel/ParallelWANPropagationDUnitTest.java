...
public class ParallelWANPropagationDUnitTest extends WANTestBase {
  ...
  public void test_ParallelGatewaySenderMetaRegionNotExposedToUser_Bug44216() {
    ...
    createPartitionedRegion(getTestMethodName() + "_PR", "ln", 1, 100);
  ...
  public void testParallelPropagationWithOverflow() {
    ...
    createCacheInVMs(nyPort, vm2, vm3);
    vm2.invoke(
        () -> WANTestBase.createPartitionedRegion(getTestMethodName(), null, 1, 100));
    vm3.invoke(
        () -> WANTestBase.createPartitionedRegion(getTestMethodName(), null, 1, 100));
    createReceiverInVMs(vm2, vm3);
    createCacheInVMs(lnPort, vm4, vm5, vm6, vm7);
    ...
    vm4.invoke(
        () -> WANTestBase.createPartitionedRegion(getTestMethodName(), "ln", 1, 100));
    vm5.invoke(
        () -> WANTestBase.createPartitionedRegion(getTestMethodName(), "ln", 1, 100));
    vm6.invoke(
        () -> WANTestBase.createPartitionedRegion(getTestMethodName(), "ln", 1, 100));
  ...
  public void testPartitionedParallelPropagationHA() throws Exception {
    ...
    vm4.invoke(
        () -> WANTestBase.createPartitionedRegion(getTestMethodName() + "_PR", "ln", 2, 100));
    vm5.invoke(
        () -> WANTestBase.createPartitionedRegion(getTestMethodName() + "_PR", "ln", 2, 100));
    vm6.invoke(
        () -> WANTestBase.createPartitionedRegion(getTestMethodName() + "_PR", "ln", 2, 100));
